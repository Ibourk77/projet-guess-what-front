<script>
  ...
  async function handleThemeChange(event) {
    ...
    const response = await fetch(
      `https://guess-what-back.onrender.com/theme/${selectedTheme}`
    );
    if (response.ok) {
  ...
  async function checkAnswer(answer, questionId) {
    ...
    if (questionIndex !== -1) {
      if (answer.is_good_answer) {
        ...
        themeScores[selectedTheme] += questions[questionIndex].usedHint
          ? 1.5
          : 2;
      } else {
  ...
  async function updateScore(themeId, score, hintsUsed, incorrectAnswers) {
    ...
    try {
      ...
      const response = await fetch(
        "https://guesswhat-api.onrender.com/user/play",
        {
          method: "POST",
          headers: {
...
<main>
  <div class="top-section">
    <div class="score-section">
      <span
        >Scores pour {themes.find((t) => t.id === selectedTheme)?.name ||
          "le thème sélectionné"} :</span
      >
      <div class="score-box">
  ...
  {#each questions as question}
    ...
    <div class="hint-section">
      <button on:click={() => requestHint(question.id)}
        >Veux-tu un indice ?</button
      >
    ...
    <div id={`answers-${question.id}`} class="answers-section">
      {#each question.answers as answer}
        <button on:click={() => checkAnswer(answer, question.id)}
