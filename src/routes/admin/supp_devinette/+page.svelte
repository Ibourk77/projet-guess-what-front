<script>
  ...
  async function getquestionfromtheme(id) {
    console.log(id);
    const questions = await fetch(`guess-what-back.onrender.com/theme/${id}`, {
      method: "GET",
    ...
    const actions = {
      default: async ({ request, cookies }) => {
        ...
        const deletedevi = await fetch(
          `https://guesswhat-api.onrender.com/admin/theme/${id}/riddle/${id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
    ...
    <form class="formadmin" action="" method="post">
      <ul>
        <li>
          ...
          <select
            on:change={(event) => getquestionfromtheme(event.target.value)}
            name="theme"
            id="theme_select"
          >
